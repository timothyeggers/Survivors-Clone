...
func _physics_process(delta):
	#if _is_repulsing:
		#REPEL(global_position, explode_radius / 4, repel_force)
...
#
#func MAKE_CIRCLE_SHAPE(explode_radius: float):
	#if _shape:
		#PhysicsServer2D.area_clear_shapes(area)
	#_shape = CircleShape2D.new()
	#_shape_rid = PhysicsServer2D.circle_shape_create()
	#PhysicsServer2D.shape_set_data(_shape_rid, explode_radius)
	#PhysicsServer2D.area_clear_shapes(area.get_rid())
	#PhysicsServer2D.area_add_shape(area.get_rid(), _shape_rid)

#func REPEL(global_pos: Vector2, explode_radius: float, force: float,  distance_magnitude := false):
#
	#MAKE_CIRCLE_SHAPE(explode_radius)
#
...
func explode(at: Vector2, force, distance_magnitude, target: VelocityComponent):
	...
	if distance_magnitude:
		var amount =  1 - (dir.length() / explode_radius)
